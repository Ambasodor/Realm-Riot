...
public class Resource implements Serializable {
    ...
    static {
        for (Class<?> cl : dolda.jglob.Loader.get(LayerName.class).classes()) {
            ...
            if (LayerFactory.class.isAssignableFrom(cl)) {
                //idk this change cuz its showing me error even tho its not
    ...
    private void load(InputStream st) throws IOException {
        ...
        if (this.ver == -1)
            ...
            if (ignoreResVersionWarningList.stream().noneMatch(name::matches)) {
				if (showResourceConsolePrints)
