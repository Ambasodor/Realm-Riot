...
Account[] accounts = [
		...
		//["Angler2Baton", "taw36gT32", "ANGLER", "https://i.imgur.com/Hb7amLs.png"]
...
def GoToStart(def coords){
a = player.SdistanceBetweenCoords(coords)
...
def findAnimal(def animalName, def character, def image) {
	...
	if (animals != 0) {
		log.info("[In-game time: ${time}] Found ${animals} ${animalName}.")
				player.discordSay("<@&1003623785058017340> [In-game time: ${time}] ${chara} found ${animals} ${animalName} ${image}.")
	...
	if (animals == 0) {
		...
		if (animals != 0) {
			...
			player.discordSay("<@&1003623785058017340> [In-game time: ${time}] ${chara} found ${animals} ${animalName} ${image} in second check.")
		}
	}
	return animals != 0
	//---------------------------------------------------------------------------------------- + /Stop second check
